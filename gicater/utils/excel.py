...
class _Wookbook(Workbook):
    ...
    ws=wb.add_sheet(_(u'sheet name'))
    ws.write(0,0,u"内容",style)
      
    response=HttpResponse(wb.save_stream(),mimetype='application/vnd.ms-excel')
    response['Content-Disposition'] = 'attachment; filename=%s.xls'%(_(u'Excel Name').encode('gbk'))
    return response
    '''
